...
/**
 * ACCOUNT — a Module Federation *remote*, loaded DYNAMICALLY.
 *
 ...
 *
 * WHAT MAKES THIS ONE DIFFERENT: nothing in *this* file. The remote is built
 * exactly like catalog and cart. The difference is entirely on the host side —
 * the shell does not declare `account` in its `remotes` block at all, and instead
 * discovers this URL at runtime from /remotes.json. That asymmetry is the point:
 * a remote cannot tell, and should not care, whether it was wired statically or
 * registered dynamically.
 *
 * That is why this app has two modes:
 *   pnpm --filter @shop/account watch    -> vite build --watch, real remoteEntry.js
 *                                           for the shell to consume (no HMR)
 *   pnpm --filter @shop/account dev      -> plain vite dev of src/standalone.tsx,
